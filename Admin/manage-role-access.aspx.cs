...
<div class='form-check form-switch changeAccess' id='v_" + rl.PageId + @"'  data-id='" + rl.PageId + @"'>
    ...
    public static string ChangeStatus(string pid, string rid, string v, string a, string e, string d)
    {
        ...
        try
        {
            ...
            if (users.Count > 0)
            {
                if (CreateUser.CheckAccess(conSQ, "manage-role-access.aspx", "Edit", HttpContext.Current.Request.Cookies["sq_aaid"].Value))
                {
                    ua.PageId = pid;
                    ua.RoleId = rid;
            ...
            {
                if (CreateUser.CheckAccess(conSQ, "manage-role-access.aspx", "Add", HttpContext.Current.Request.Cookies["sq_aaid"].Value))
                {
                    ua.PageId = pid;
                    ua.RoleId = rid;
                    ua.AddAccess = a == "Y" ? "1" : "0";
