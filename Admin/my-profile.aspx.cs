...
public partial class Admin_my_profile : System.Web.UI.Page
{
    ...
    public void GetProfileDetails()
    {
        try
        {
            List<CreateUser> comps = CreateUser.GetAllUser(conSQ).Where(x => x.UserGuid == Request.Cookies["sq_aaid"].Value).ToList();
            if (comps.Count > 0)
            {
                btnSave.Text = "Update";
    ...
    public string UploadProfileImage()
    {
        ...
        if (fuProfile.HasFile)
        {
            ...
            string iconPath = Server.MapPath(".") + "\\../ShopifyUploadImage\\" + ImageGuid1 + "" + fileExtension;
            if ((fileExtension == ".jpg" || fileExtension == ".jpeg" || fileExtension == ".png" || fileExtension == ".gif" || fileExtension == ".webp"))
            {
                ...
                thumbImage = "ShopifyUploadImage/" + ImageGuid1 + "" + fileExtension;
            }
            else
            {
                return "Format";
        ...
        try
        {
            if (Page.IsValid)
            {
                ...
                if (CreateUser.CheckAccess(conSQ, pageName, "Edit", Request.Cookies["sq_aaid"].Value))
                {
                    ...
                    comp.UserGuid = Request.Cookies["sq_aaid"].Value;
                    comp.AddedBy = Request.Cookies["sq_aaid"].Value;
