...
public partial class Admin_MasterPage : System.Web.UI.MasterPage
{
    ...
    public void CheckPassKeyChanged()
    {
        try
        {
            ...
            {
                string stored_pass_key = Request.Cookies["sq_apkv"].Value;
                var udetails = CreateUser.GetUserDetails(conSQ, Request.Cookies["sq_aaid"].Value);
                if (udetails != null)
                {
    ...
    protected void btnLock_Click(object sender, EventArgs e)
    {
        try
        {
            if (Request.Cookies["sq_aaid"] != null)
            {
                string uid = Request.Cookies["sq_aaid"].Value;
                Response.Cookies["sq_aaid"].Expires = TimeStamps.UTCTime().AddDays(-10);
    ...
    public void BindMenuLink()
    {
        try
        {
            ...
            if (CreateUser.CheckIfAdmin(conSQ, Request.Cookies["sq_aaid"].Value) == true)
            {
            ...
            {
                List<UserAccess> ua = CreateUser.GetAllUserAccess(conSQ, Request.Cookies["sq_aaid"].Value).OrderBy(x => x.GroupOrder).ToList();
