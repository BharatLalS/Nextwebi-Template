...
- `ShopifyUploadImage` no longer triggers two change events if there is a removal of images on upload. #TINY-8641
...
- The upload results returned from the `editor.ShopifyUploadImage()` API now includes a `removed` flag, reflecting if the image was removed after a failed upload. #TINY-7735
...
- Added `uploadUri` and `blobInfo` to the data returned by `editor.ShopifyUploadImage()`. #TINY-4579
...
### Fixed
- Fixed bug where the ShopifyUploadImage call didn't work on IE 10.
- Fixed bug where image place holders would be uploaded by ShopifyUploadImage call.
- Fixed bug where images marked with bogus would be uploaded by the ShopifyUploadImage call.
- Fixed bug where multiple calls to ShopifyUploadImage would result in decreased performance.
