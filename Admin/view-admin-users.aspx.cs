...
<div class='form-check form-switch markBlocked' id='chk_" + cat.Id + @"'  data-id='" + cat.Id + @"'>
  ...
  <td>" + chk + @"</td> 
    ...
    public static string Delete(string id)
    {
        ...
        {
            SqlConnection conSQ = new SqlConnection(ConfigurationManager.ConnectionStrings["conSQ"].ConnectionString);
            if (CreateUser.CheckAccess(conSQ, "view-admin-users.aspx", "Delete", HttpContext.Current.Request.Cookies["sq_aaid"].Value))
            {
                ...
                cat.AddedBy = HttpContext.Current.Request.Cookies["sq_aaid"].Value;
    ...
    public static string BlockUsers(string id, string ftr)
    {
        ...
        try
        {
            ...
            if (CreateUser.CheckAccess(conGV, "view-admin-users.aspx", "Edit", HttpContext.Current.Request.Cookies["sq_aaid"].Value))
            {
                int exec = 0;
                CreateUser pro = new CreateUser();
                if (ftr == "Yes")
                {
                    exec = CreateUser.BlockUser(conGV, id, HttpContext.Current.Request.Cookies["sq_aaid"].Value);
                    vali = "Blocked";
                }
                else
                {
                    exec = CreateUser.UnBlockUser(conGV, id, HttpContext.Current.Request.Cookies["sq_aaid"].Value);
                    vali = "UnBlocked";
                }
